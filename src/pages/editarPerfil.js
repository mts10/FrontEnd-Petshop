...
export default function EditarPerfil({ clienteId }) {
  ...
  useEffect(() => {
    const carregarCliente = async () => {
      try {
        const response = await api.get(`/auth/${clienteId}`);
    ...
    <div className="container">
      ...
      <form action="/clientes" method="post" encType="multipart/form-data" onSubmit={handleSubmit}>
        <div className="row mt-5">
          ...
          <div className="col">
            ...
            <div className="mt-3">
              ...
              <input
                className="form-control"
                placeholder="Insira o nome que está no cartao"
                id="NomeCartaoInput"
                type="text"
                value={cliente.nomeCartao}
                onChange={handleChange}
              />
            </div>
            <div className="mt-3">
        ...
        <div>
          <div className="mt-5 text-center">
            <Link to="/">
              <button className="btn" onClick={handleAlertEditar}>Atualizar</button>
            </Link>
          </div>
        </div>
      </form>
